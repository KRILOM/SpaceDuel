...
public class Player extends Entity{
    ...
    private enum Heading {//перечисление направлений самолета(то куда на плеер в данный момент смотрит )
       //привязываем кажждой нумерации координаты
        NORTH(0*SPRITE_SCALE,0*SPRITE_SCALE,SPRITE_SCALE * 1,SPRITE_SCALE * 1),//умножаем на 1, чтоб не делать пересчет //сохраняем координаты первого танка смотрящего вверх
        EAST(2*SPRITE_SCALE,0*SPRITE_SCALE,SPRITE_SCALE * 1,SPRITE_SCALE * 1),//
        SOUTH(0*SPRITE_SCALE,0*SPRITE_SCALE,SPRITE_SCALE * 1,SPRITE_SCALE * 1),
        WEST(11*SPRITE_SCALE,0*SPRITE_SCALE,SPRITE_SCALE * 1,SPRITE_SCALE * 1);
    ...
    private Heading heading;//объект хранящий в себе направление самолета
    private Map<Heading, Sprite> spriteMap;//ключ это нумерейш, а значение это спрайт => мапа нужна для того чтобы правильно вытасивать из нее спрайт уже с заданным направлением
    ...
    public Player(float x, float y, TextureAtlas atlas,float scale, float speed ) {
        ...
        heading = Heading.NORTH;//изнаально любой плеер смотрит на север, то есть от игрока
        spriteMap = new HashMap<Heading, Sprite>();//на каждый хединг мы хотим сохранять отдельный спрайт
        this.scale = scale;
        this.speed = speed;


        for(Heading h: Heading.values()){// возвращает массив который хранит в себе все значения которые есть в енумерации(в данном случае наш h будет равняться каждому из значений Heading)
    ...
    public void update(Input input) {//управление поэтому передаем инпут
        ...
        if(input.getKey(KeyEvent.VK_UP)){// если кнопка навер зажата, то
            newY -= speed;
        ...
        else if(!input.getKey(KeyEvent.VK_LEFT) || !input.getKey(KeyEvent.VK_RIGHT)){
            heading = Heading.NORTH;
        }
